...
app.get("/api/pnl-image", handleImageRequest);
app.post("/api/pnl-image", handleImageRequest);
app.get("/api/pnl-image.png", handleImageRequest);
app.post("/api/pnl-image.png", handleImageRequest);
...
function formatCompactToken(value) {
  ...
  if (abs >= 1_000_000_000) {
    return `${trimZeros((value / 1_000_000_000).toFixed(2))}B SOL`;
  }

  if (abs >= 1_000_000) {
    return `${trimZeros((value / 1_000_000).toFixed(2))}M SOL`;
  }

  if (abs >= 1_000) {
    return `${trimZeros((value / 1_000).toFixed(2))}K SOL`;
  }

  return `${trimZeros(value.toFixed(2))} SOL`;
}
...
function getThemePalette(theme, isPositive) {
  if (theme === "green") {
    return {
      ...
      profit: "#5df2a2",
      loss: "#ff6b81",
      topBar: "linear-gradient(90deg, rgba(34,197,94,0) 0%, rgba(34,197,94,0.88) 48%, rgba(45,212,191,0) 100%)",
  ...
  return {
    ...
    profit: "#4ade80",
    loss: "#fb7185",
    topBar: "linear-gradient(90deg, rgba(56,189,248,0) 0%, rgba(56,189,248,0.9) 48%, rgba(14,165,233,0) 100%)",
    ...
    <head>
      ...
      <style>
        :root {
          ...
          --glass: rgba(8, 15, 28, 0.66);
          --glass-strong: rgba(7, 13, 26, 0.88);
          ...
          --line-glow: ${model.palette.lineGlow};
        }
...
function renderDocsPage() {
  ...
  <html lang="en">
    <head>
      <meta charset="UTF-8" />
      <meta name="viewport" content="width=device-width, initial-scale=1.0" />
      <title>SOLIX PnL Image API</title>
      <style>
        :root {
          color-scheme: dark;
          --bg-top: #04101f;
    ...
    <body>
      <main class="panel">
        <h1>SOLIX PnL Image API</h1>
        <p>Generate Discord-ready 1200x630 PNG PnL cards from query params or JSON payloads.</p>
        <a class="cta" href="/api/pnl-image.png?project=SOLIX&entry=2.5&startmc=2&endmc=26.5&gas=0.12&theme=blue">Open Sample Image</a>

        <section class="block">
          <div class="label">GET</div>
          <code>/api/pnl-image.png?project=SOLIX&amp;entry=2.5&amp;startmc=2&amp;endmc=26.5&amp;gas=0.12&amp;theme=blue</code>
        </section>
